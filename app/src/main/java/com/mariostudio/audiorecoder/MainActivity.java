...
    * 申请录音权限*/
    public void verifyAudioPermissions(Activity activity) {
        ...
        int writerPermission = ActivityCompat.checkSelfPermission(activity,Manifest.permission.WRITE_EXTERNAL_STORAGE);

        if (permission != PackageManager.PERMISSION_GRANTED && writerPermission != PackageManager.PERMISSION_GRANTED) {
